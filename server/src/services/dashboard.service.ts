...
export const getStudentDashboard = async (userId: string) => {
  ...
  const totalCreditsPlanned = allCourses.reduce(
  ...
  allCourses.forEach((course) => {
    if (course.category) {
      creditsByCategory[course.category] =
        (creditsByCategory[course.category] || 0) + (course.credits || 0);
    }
  });

  const categoryDistribution = Object.entries(creditsByCategory).map(
    ([name, value]) => ({
      category: name.replace(/_/g, " "),
      credits: value,
      percentage:
        totalCreditsPlanned > 0
          ? Math.round((value / totalCreditsPlanned) * 100)
          : 0,
    })
  );
  ...
  const categoryProgress = Object.entries(categoryRequirements).map(
    ([category, required]) => {
      const earned = creditsByCategory[category] || 0;
      const percentage =
        required > 0 ? Math.round((earned / required) * 100) : 0;
...
export const getMentorDashboard = async (mentorId: string) => {
  ...
  const menteesProgress = mentees.map((mentee) => {
    ...
    const expectedCredits =
      mentee.classification === Classification.FRESHMAN
        ? 30
        : mentee.classification === Classification.SOPHOMORE
        ? 60
        : mentee.classification === Classification.JUNIOR
        ? 90
        : 120;
  ...
  const menteesProgressData = [
    {
      name: "On Track",
      value: menteesProgress.filter((p) => p === "On Track").length,
    },
    {
      name: "Slightly Behind",
      value: menteesProgress.filter((p) => p === "Slightly Behind").length,
    },
    {
      name: "Significantly Behind",
      value: menteesProgress.filter((p) => p === "Significantly Behind").length,
    },
    {
      name: "Ahead of Schedule",
      value: menteesProgress.filter((p) => p === "Ahead of Schedule").length,
    },
  ];
  ...
  const planStatusData = Object.entries(planStatusCounts).map(
    ([name, value]) => ({
      name,
      value,
    })
  ...
  mentees.forEach((mentee) => {
    mentee.degreePlan?.semesters.forEach((semester) => {
      semester.plannedCourses.forEach((course) => {
        if (course.category) {
          aggregateCredits[course.category] =
            (aggregateCredits[course.category] || 0) + (course.credits || 0);
        }
      });
    });
  });

  const aggregateCreditsData = Object.entries(aggregateCredits).map(
    ([name, value]) => ({
  ...
  const riskLevelData = [
    {
      name: "Low Risk",
      value: menteesProgress.filter(
        (p) => p === "On Track" || p === "Ahead of Schedule"
      ).length,
    },
    {
      name: "Medium Risk",
    ...
    },
    {
      name: "High Risk",
      value: menteesProgress.filter(
        (p) => p === "Significantly Behind" || p === "No Plan Created"
      ).length,
    },
  ];
  ...
  const decisionOutcomes = [
    {
      name: "Approved",
      value: reviewRequests.filter(
        (r) =>
          r.status === ReviewStatus.PENDING_ADVISOR ||
          r.status === ReviewStatus.APPROVED
      ).length,
    },
    {
      name: "Rejected",
      value: reviewRequests.filter((r) => r.status === ReviewStatus.REJECTED)
        .length,
    },
    {
      name: "Sent Back for Edits",
      value: reviewRequests.filter((r) => r.rejectionReason).length,
    },
  ];
...
export const getAdvisorDashboard = async (advisorId: string) => {
  ...
  const studentPlanStatusData = Object.entries(planStatusCounts).map(
    ([name, value]) => ({
      name,
      value,
    })
  ...
  const advisorDecisionData = [
    {
      name: "Approved",
      value: reviewRequests.filter((r) => r.status === ReviewStatus.APPROVED)
        .length,
    },
    {
      name: "Rejected",
    ...
    {
      name: "Returned for Changes",
      value: reviewRequests.filter(
        (r) => r.advisorComment && r.status === ReviewStatus.PENDING_ADVISOR
      ).length,
    },
  ];
  ...
  const classificationData = [
    {
      ...
      value: students.filter(
        (s) => s.classification === Classification.FRESHMAN
      ).length,
    },
    {
      name: "Sophomore",
      value: students.filter(
        (s) => s.classification === Classification.SOPHOMORE
      ).length,
    },
    {
      name: "Junior",
      value: students.filter((s) => s.classification === Classification.JUNIOR)
        .length,
    },
    {
      name: "Senior",
      value: students.filter((s) => s.classification === Classification.SENIOR)
        .length,
    },
  ];
...
export const getRegistrarDashboard = async () => {
  ...
  const programCreditsData = [
    {
      name: "< 120 credits",
      value: programs.filter((p) => p.totalCredits < 120).length,
    },
    {
      name: "120-130 credits",
      value: programs.filter(
        (p) => p.totalCredits >= 120 && p.totalCredits <= 130
      ).length,
    },
    {
      name: "> 130 credits",
      value: programs.filter((p) => p.totalCredits > 130).length,
    },
  ];
...
export const getAdminDashboard = async () => {
  ...
  const systemActivityData = [
    {
      name: "Active Plans",
      value: degreePlans.filter((p) =>
        p.semesters.some((s) =>
          s.reviewRequests.some((r) => r.status === ReviewStatus.APPROVED)
        )
      ).length,
    },
    {
      name: "Draft Plans",
      value: degreePlans.filter(
        (p) => !p.semesters.some((s) => s.reviewRequests.length > 0)
      ).length,
    },
    {
      name: "No Plan Yet",
      value:
        (await prisma.user.count({ where: { role: Role.STUDENT } })) -
        degreePlans.length,
    },
  ];
  ...
  const approvalWorkflowData = [
    {
      name: "Pending Mentor",
      value: allReviewRequests.filter(
        (r) => r.status === ReviewStatus.PENDING_MENTOR
      ).length,
    },
    {
      name: "Pending Advisor",
      value: allReviewRequests.filter(
        (r) => r.status === ReviewStatus.PENDING_ADVISOR
      ).length,
    },
    {
      name: "Fully Approved",
      value: allReviewRequests.filter((r) => r.status === ReviewStatus.APPROVED)
        .length,
    },
    {
      name: "Rejected",
      value: allReviewRequests.filter((r) => r.status === ReviewStatus.REJECTED)
        .length,
    },
  ];
