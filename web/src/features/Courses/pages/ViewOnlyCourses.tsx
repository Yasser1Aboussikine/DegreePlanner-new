...
const ViewOnlyCourses = () => {
  const {
    ...
    page: 1,
    limit: 1000, // Fetch a large number of courses to get all of them
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <p>
          ...
          {hasActiveFilters &&
            ` (filtered from ${courses?.data?.length || 0} total)`}
        </p>
      </div>
...
const CourseCard = ({ course }: { course: Course }) => {
  ...
  const basePath = userRole === "MENTOR" ? "/mentor" : "/student";
...
export default ViewOnlyCourses;
