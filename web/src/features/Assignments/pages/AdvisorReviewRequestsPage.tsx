...
export const AdvisorReviewRequestsPage = () => {
  const navigate = useNavigate();
  const user = useAppSelector((state) => state.auth.user);
  const { data, isLoading, refetch } = useGetPendingAdvisorReviewsQuery(
    user?.id || "",
    {
      skip: !user?.id,
    }
  );
  const [submitBulkAdvisorReview, { isLoading: isSubmitting }] =
    useSubmitBulkAdvisorReviewMutation();
  ...
  const handleSubmitReview = async () => {
    ...
    // Only require general rejection reason when rejecting
    if (!isApproving) {
      if (!generalRejectionReason.trim()) {
    ...
    console.log("[DEBUG] Selected requests:", selectedRequests);
    console.log("[DEBUG] First request:", selectedRequests[0]);
    console.log("[DEBUG] Plan semester:", selectedRequests[0]?.planSemester);
    console.log("[DEBUG] Degree plan ID:", degreePlanId);
    ...
    console.log("[DEBUG] Submitting bulk advisor review:", payload);

    try {
      await submitBulkAdvisorReview(payload).unwrap();

      toast.success(
        isApproving
          ? "Degree plan fully approved!"
  ...
  const handleViewDegreePlan = (studentId: string) => {
    navigate(`/advisor/students/${studentId}/degree-plan`);
  };
    ...
    <div className="container mx-auto p-6 bg-background">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-2">
          Review and approve degree plans from your assigned students (approved
          by mentors)
        </p>
      </div>
      ...
      {Object.keys(groupedRequests).length === 0 ? (
        ...
        <div className="space-y-6">
          {Object.entries(groupedRequests).map(([studentId, requests]) => {
            ...
            const mentor = requests[0]?.mentor;
            const mentorComment = requests[0]?.mentorComment;
                ...
                {mentor && (
                  <div className="mb-4 p-3 bg-muted rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">
                      Approved by mentor: <strong>{mentor.name}</strong>
                    </p>
                    {mentorComment && (
          ...
          <DialogFooter>
            ...
            >
              {isSubmitting ? (
                ...
                <>{isApproving ? "Final Approval" : "Reject"}</>
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
